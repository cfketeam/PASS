...
#' fit <- pass.glm(x=a$x, y=a$y,sam.time=10,seed=101,alpha=3,bar=0.01,weight=0.9)
#' fit
#'

#Model Size search#

pass.glm <- function(x,y,bar=0.01,method="mean",n.max=NULL,sam.time=10,seed=101,alpha=3,weight=0.8,sim.percent=0.5)
{
  re <- list()
  rlist <- c()
  numlist <- c()
  ...
  {
    kr <- n_min
    rr <- glm.fixed(x=x,y=y,n=n_min,sam.time=sam.time,weight=weight,sim.percent=sim.percent,seed=seed)
    auc_mean <- c(rr$performance)
    auc_tmean <- c(rr$other)
    ms <- paste0("Model size: 1")
    if(method=="mean")
      pf <- paste0("  AUC: ",format(auc_mean[length(auc_mean)], nsmall = 3,digits = 3))
    if(method=="Tmean")
      pf <- paste0("  AUC: ",format(auc_tmean[length(auc_tmean)], nsmall = 3,digits = 3))
    cat(paste0(ms,pf),"\n")
    ...
    while(min(s_tmean,s_mean)<2 && kr<n_max)
    {
      ...
      kl <- kr
      kr <- kr+1
      rr <- glm.fixed(x=x,y=y,sam.time=sam.time,n=kr,weight=weight,sim.percent=sim.percent,seed=seed)
    ...
    re <- list(n=length(var),subset=best$subset, bestmodel=bestmodel,performance=performance,
               performance.list=performance.list,othersubset=other$subset,otherperformance=other$other
